...
export const loader = async () => {
  const products = await prisma.product.findMany(); // lấys sản phẩm
...
export default function ProductList() {
  const products = useLoaderData(); // Lấy dữ liệu sản phẩm
  ...
  return (
    <div>
      ...
      <details className="mb-4">
        <summary className="cursor-pointer text-blue-500">+ Add New Product</summary>
        {/* Form thêm sản phẩm */}
        <Form method="post" className="mt-4 space-y-4">
          {actionData?.error && <p className="text-red-500">{actionData.error}</p>}
          <div>
            <label htmlFor="name" className="block">Product Name</label>
            <input type="text" name="name" id="name" required className="border rounded px-2 py-1" />
          </div>
          <div>
            <label htmlFor="description" className="block">Description</label>
            <textarea name="description" id="description" className="border rounded px-2 py-1" />
          </div>
          <div>
            <label htmlFor="url" className="block">Product Image URL</label>
            <input type="text" name="url" id="url" className="border rounded px-2 py-1" />
          </div>
      ...
      <ul className="space-y-4">
        {products.map((product) => (
          <li key={product.id} className="border p-4 rounded-lg shadow-lg">
            ...
            {product.url && <img src={product.url} alt={product.name} width="200" className="mt-2" />}
